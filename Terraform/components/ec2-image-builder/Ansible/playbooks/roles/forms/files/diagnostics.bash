...
HS_LOG_LINES_TO_SAVE="${HS_LOG_LINES_TO_SAVE:-all}"
...
HS_APP_INFO_SUB="${HS_APP_INFO_SUB:-system_settings_export,layout_release,usage_report,trained_models_metadata,threshold_audits,machine_audit_logs,attached_trainers,latest_trainer_runs,latest_jobs,get_connector_logs,get_configs,init_entries,health_records,health_statistics_records,workflow_instances,workflow_dsls,db_info}"
    ...
    if [[ "${has_docker_access}" == "0" ]]; then
        if [[ $(id -u) -ne 0 ]]; then
            ...
            if [[ "${NO_SUDO}" == "true" ]]; then
                myecho "[WARNING] Has docker access but not run with sudo. Please re-run with sudo for best results."
            else
                # Docker, no sudo
                # bypass myecho and force stdout
                echo "[WARNING] Has docker access but not run with sudo. Please re-run with sudo for best results or with NO_SUDO=true."
                exit 1
            fi
        # Docker, sudo
        else
        ...
        # No docker
        echo "[ERROR] No docker access. Please re-run with sudo."
        exit 1
    fi
}
...
function clean_up_previous_runs() {
    container=$(docker ps --format '{{.Names}}' | grep "${HS_APP_INFO_CONTAINER}")
    ...
    docker exec -it ${container} rm -f /tmp/code.py
...
function capture_docker_daemon_info() {
    ...
    myecho "checking docker system df -v"
    docker system df -v >./docker/docker_system_df.txt 2>&1

    myecho "checking docker ps -a -s"
    docker ps -a -s >./docker/docker_ps.txt 2>&1

    myecho "checking docker system events"
    docker system events --since 300h --until 0m >./docker/docker_system_events.txt 2>&1

    myecho "checking docker info"
    docker info >./docker/docker_info.txt 2>&1
...
function capture_container_info() {
    ...
    docker logs --tail ${HS_LOG_LINES_TO_SAVE} $container 2>&1 |
        gzip \
    ...
    if docker ps | grep -q -w $container; then
    ...
    docker cp $container:/var/log ./containers/${container}/var_log
    ...
    myecho "docker inspect $container"
    docker inspect "${container}" |
        docker exec -i "${container}" /var/www/venv/bin/python -c 'import json; import sys; d = json.load(sys.stdin); d[0]["Config"].pop("Env", None); print(json.dumps(d, indent=4))' \
        > "./containers/${container}/inspect.json"

    docker exec -i "${container}" bash -c "cd /var/www/forms/forms; /var/www/venv/bin/python -c 'import json; from common.system_variable import SystemVariable; print(json.dumps(SystemVariable.redacted_app_variables(), indent=4))'" \
        > "./containers/${container}/app_variables.json"

    docker exec -i "${container}" bash -c "cd /var/www/forms/forms; /var/www/venv/bin/python -c 'import json; from common.system_variable import SystemVariable; print(json.dumps(SystemVariable.redacted_env_variables(), indent=4))'" \
        > "./containers/${container}/env_variables.json"

    myecho "certs directory within $container"
    docker exec -it "${container}" ls -R "${IN_DOCKER_CERTS_DIR}"

    eval $(docker inspect -f '{{range .Config.Env}}{{printf "%s\n" .}}{{end}}' "${container}" | grep FORMS_STORAGE_MODE)
    ...
    {
        test_file_name="${container}_$(date "+%Y-%m-%d-%H%M%S%Z")_access"
        host_input=$(docker inspect -f '{{range .Mounts}}{{printf "%s\n%s\n" .Source .Destination}}{{end}}' "${container}" | grep -v "${IN_DOCKER_INPUT_DIR}" | grep "/input$")
        if [[ ! -z "$host_input" ]]; then
            docker exec -it "${container}" touch "${IN_DOCKER_INPUT_DIR}/${test_file_name}"
            test -f "${host_input}/${test_file_name}" || myecho "Input directory out of sync between ${container} and host"
            docker exec -it "${container}" rm "${IN_DOCKER_INPUT_DIR}/${test_file_name}"
        else
            myecho "Not checking input dir access because input dir is not set"
        fi
    } >./containers/${container}/input_test.txt 2>&1

    docker exec -i "${container}" cat /etc/resolv.conf > "./containers/${container}/resolvconf.txt"

    docker exec -i "${container}" cat /etc/nsswitch.conf > "./containers/${container}/nsswitchconf.txt"
}
...
function slurp_stderr_fifo() {
    while read -r line; do
        if [[ "${line}" =~ ${MAGIC_HEADER}(.*) ]]; then
            ...
            docker exec -i "${container}" bash -c "echo \"${CODE}\" > /tmp/run.py; echo \"write_file()\" >> /tmp/run.py; /var/www/venv/bin/python /tmp/run.py; rm /tmp/run.py" >"${file_name}" <"${FIFO_OUT}"
            # Continue signal
            echo > "${FIFO_IN}"
        else
            myecho "${line}" >&2
...
function capture_app_info() {
    ...
    if [[ "$IS_INSIDE_CONTAINER" == "false" ]]; then
        container=$(docker ps --format '{{.Names}}' | grep "${HS_APP_INFO_CONTAINER}")
        application_version=$(docker exec -it "${container}" bash -c $'cat forms/forms/FORMS_VERSION  | /var/www/venv/bin/python -c "import sys, json; print(json.load(sys.stdin)[\'version\'], end=\'\')"')
    else
...
"
    ...
    if [[ "${IS_INSIDE_CONTAINER}" == "true" ]]; then
        ...
        pid=$!
        disown
        docker exec ${docker_env_vars[@]} -i "${container}" bash -c "unset SENTRY_DSN; source /var/www/venv/bin/activate; cd /var/www/forms/forms/; echo \"${code}\" > /tmp/hs_diag.py ; python /tmp/hs_diag.py" < ${FIFO_IN} 2>${FIFO_OUT} >> docker_exec.log &
        slurp_stderr_fifo 2>&1 | tee -a docker_exec.log
        # Close FIFO_IN pipe to end docker exec
        kill $pid
    else
        docker exec ${docker_env_vars[@]} -it "${container}" bash -c "unset SENTRY_DSN; source /var/www/venv/bin/activate; cd /var/www/forms/forms/; echo \"${code}\" > /tmp/hs_diag.py; python -i /tmp/hs_diag.py" 2>/dev/null
    fi
}
...
def capture_latest_jobs():
    ...
    jobs = serializer.annotate_queryset(ExtractionJobQueueViewSet.queryset)
    ...
    for i in range(JOBS_PERIOD_IN_DAYS):
        write_json(
            JOBS_FNAME_FMT % i,
            ExtractionJobQueueViewSet.serializer_class(
                jobs.filter(created__gte=created_beg, created__lt=created_end), many=True
            ).data,
        )
...
def capture_workflow_instances():
    ...
    def get_workflows_json(ids_in_batch):
        sem.acquire()
        try:
            unique_dsls = {}
            wfis = []
            for workflow_id in ids_in_batch:
                try:
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        correlation_ids = list(WfeWorkflowInstance.objects.filter(
                dt_started__gte=WORKFLOW_EXPORT_LATEST_DT - timedelta(seconds=WORKFLOW_EXPORT_PERIOD_IN_SECS),
                dt_started__lt=WORKFLOW_EXPORT_LATEST_DT,
        ).values_list('correlation_id', flat=True).distinct()[:DISTINCT_CORRELATION_IDS_LIMIT])
        ...
        for correlation_id in correlation_ids:
            ...
            futures.append(executor.submit(get_workflows_json, workflow_ids))
